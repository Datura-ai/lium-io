...
| `executor` | built from `neurons/executor` | the real executor: FastAPI on :8001 (`/version`, `/upload_ssh_key`, …), sshd on :22, `DOCKER_HOST` → dind. Trusts the e2e validator through `executor/config_override.py` (the mechanism the `:dev` images use) and the e2e miner through `MINER_HOTKEY_SS58_ADDRESS` |
