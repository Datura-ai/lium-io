...
No application code: nothing here imports the validator or the backend. `PROTOCOL_VERSION` (`1.1.0`) is semver over the wire; 1.1.0 added `PodStatesReport`, `ExecutorSpecRequest.pod_states` and `FailedContainerErrorCodes.ExecutorUnreachable` (per-cycle pod container states, and "unreachable" told apart from "unknown id").
