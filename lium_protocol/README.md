...
- `1.1.0` — `FailedContainerRequest.build_log_tail` (optional; the last lines a failed custom-Dockerfile build printed, lium-io#1376) and `FailedContainerRequest.step_detail` (optional; the Docker daemon's reason for a failed volume step, or the dead-transport hint, lium-io#1377). lium-platform reads them on its backend subclass until it re-vendors this tag.
- `1.0.0` — first release.
