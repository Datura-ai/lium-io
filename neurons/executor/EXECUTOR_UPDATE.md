...
2. When the runner container runs a different digest, the updater first makes sure that the host can start a GPU container: it starts a throwaway container from the runner's own image with every GPU requested and the entrypoint `true`, then removes it. The throwaway container carries the label `io.lium.watchtower.probe=nvidia-runtime`. Before the updater starts a new one, it removes every container with that label that an earlier check left behind, so a held host never collects them. If the daemon cannot start that container, the updater does not pull. It logs `Update held` with the reason and tries again at the next check. The most common reason is `NVIDIA_RUNTIME_MISMATCH`: the host's NVIDIA driver and its libraries no longer match, which happens after a driver package update without a reboot. A new executor container cannot start on such a host, so an update would take the node offline.
3. When the GPU container starts, the updater pulls `daturaai/compute-subnet-executor-runner@<digest>` and recreates the runner from it with the same name, labels, mounts and restart policy.
4. The runner starts the executor containers from the compose file that is baked into its image. That file pins the executor image by digest.
...
A line `Pull by digest failed through the daemon's registry path, retrying from Docker Hub directly` means the mirror could not serve the digest and the updater went to Docker Hub itself. The next lines say whether that worked. A line `Failed to fetch digest from endpoint` means the host cannot reach `lium.io`. Make sure that the outbound firewall allows `lium.io`. A line `Signature verification failed` means the clock is more than 10 minutes off, or the response did not come from the validator. Make sure that `date -u` is correct. A line `Runner container not identified this cycle, nothing pulled` means two containers carry the runner label. That happens for a moment during `docker compose up -d` and clears at the next check. If it stays, `docker ps -a --filter label=com.docker.compose.service=executor-runner` lists both. A container named `executor-executor-runner-1-previous-<id>` is the old runner of an update that was interrupted between its rename and its removal. The updater finishes that update on its own: a line `Starting the runner left behind by an interrupted update` means the runner under the real name was not running and was started; a line `Removing leftover runner container from an interrupted update` means the leftover was removed; a line `Restoring the name of a leftover runner container from an interrupted update` means the leftover was the only runner and got its name back. A line `Leftover runner container kept: the runner is not running` means the runner under the real name did not start (the line carries the error); the updater keeps both and goes on with the digest check. Then run `docker compose up -d`, which recreates the runner, and remove the leftover with `docker rm -f <name>`. A line `Update held: the NVIDIA runtime cannot start a GPU container on this host` means the host cannot start a GPU container right now, so the updater waits. The pods that already run on the host keep running; the hold exists so that they do. With `"reason_code": "NVIDIA_RUNTIME_MISMATCH"` the host's NVIDIA driver and its libraries do not match. The fix is a reboot, or a driver reload, and both stop every running pod. First click **Pause New Rentals** on the node in the Provider Portal: the node then takes no new rental, and the pod that runs now is not touched. Wait until the rental on this node ends. Then reboot the host or reload the driver, then run `docker compose up -d` here. The update runs at the next check after that. With `"reason_code": "RUNTIME_PROBE_FAILED"` the `error` field carries the daemon's message. To see the same failure by hand, start the probe the way the updater does, from the runner image that is already on the host: `docker run --rm --gpus all --entrypoint true "$(docker inspect executor-executor-runner-1 --format '{{.Image}}')"`. Do not use `ubuntu` or another image name here: a name is pulled from the registry, and a registry error then hides the runtime error. A line `Removing a stale runtime probe container` means an earlier check could not remove its throwaway container and this check removed it; nothing to do. A line `Update held` whose `error` starts with `stale runtime probe container not removed` means the daemon refuses to remove that container: `docker ps -a --filter label=io.lium.watchtower.probe` lists it, `docker rm -f <id>` removes it. If the log does not say, send the 50 lines to Lium support with the node address.
