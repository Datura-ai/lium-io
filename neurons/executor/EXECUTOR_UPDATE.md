...
2. When the runner container runs a different digest, the updater first makes sure that the host can start a GPU container: it starts a throwaway container from the runner's own image with every GPU requested and the entrypoint `true`, then removes it. If the daemon cannot start that container, the updater does not pull. It logs `Update held` with the reason and tries again at the next check. The most common reason is `NVIDIA_RUNTIME_MISMATCH`: the host's NVIDIA driver and its libraries no longer match, which happens after a driver package update without a reboot. A new executor container cannot start on such a host, so an update would take the node offline.
