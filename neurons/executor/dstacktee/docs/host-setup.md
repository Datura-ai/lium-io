...
- **GPU**: NVIDIA Confidential-Computing capable — Hopper (H100/H200) or Blackwell (B200/B300). Set the mode with [gpu-admin-tools](https://github.com/NVIDIA/gpu-admin-tools); every mode switch resets the GPU. Single GPU per CVM: CC mode (`--set-cc-mode=on`, after `--set-ppcie-mode=off`). All GPUs of an HGX Hopper node in one CVM (8×H100/8×H200): Protected PCIe on the GPUs **and** NVSwitches (`--set-cc-mode=off` first, then `--set-ppcie-mode=on` with `--devices gpus,nvswitches`), and `CVM_GPUS=all` — a subset is not supported, and a listed set of addresses leaves the NVSwitches on the host; per-card CC isolates each GPU and the driver disables NVLink. Blackwell: CC mode only, Protected PCIe does not exist there. Full procedure: [Enable GPU Confidential Computing mode](https://docs.lium.io/providers/nodes/cvm#4-enable-gpu-confidential-computing-mode) in the public CVM guide.
