...
def test_verify_signed_digest_checks_the_validator_signature_over_digest_and_timestamp():
    ...
    with patch.object(svc, "match_validator_hotkey", return_value="current") as match:
        digest = verify_signed_digest({"digest": NEW_DIGEST, "timestamp": 1_000_000, "signature": "0xabc"}, now=1_000_100)
    assert digest == NEW_DIGEST
    # the check goes through the executor's one validator-hotkey list (DAH-3394: current + next)
    match.assert_called_once_with(f"{NEW_DIGEST}:1000000", "0xabc")
...
def test_verify_signed_digest_rejects_a_bad_signature_and_a_stale_timestamp():
    ...
    with patch.object(svc, "match_validator_hotkey", return_value=None):
        with pytest.raises(ValueError, match="signature"):
            verify_signed_digest({"digest": NEW_DIGEST, "timestamp": 1_000_000, "signature": "0xabc"}, now=1_000_000)
    with patch.object(svc, "match_validator_hotkey", return_value="current"):
        with pytest.raises(ValueError, match="timestamp"):
            verify_signed_digest({"digest": NEW_DIGEST, "timestamp": 1_000_000, "signature": "0xabc"}, now=1_000_000 + 601)


def test_verify_signed_digest_accepts_the_next_validator_hotkey_during_a_rotation(monkeypatch):
    """Regression (DAH-3394 x DAH-3419): the digest check trusted the single built-in hotkey
    only, so once the validator signs with its ``next`` hotkey every node would report the
    signed digest as unavailable and never see an update pending. The check reads the same
    ``current``/``next`` list as every other validator-signature check."""
    import dependencies.auth as auth

    current, next_ = MagicMock(), MagicMock()
    current.verify.return_value = False
    next_.verify.return_value = True
    keypairs = {"5Fcurrent": current, "5Fnext": next_}
    monkeypatch.setattr(auth, "VALIDATOR_HOTKEYS_SS58", {"current": "5Fcurrent", "next": "5Fnext"})
    with patch.object(auth.bittensor, "Keypair", side_effect=lambda ss58_address: keypairs[ss58_address]):
        digest = verify_signed_digest({"digest": NEW_DIGEST, "timestamp": 1_000_000, "signature": "0xabc"}, now=1_000_000)
    assert digest == NEW_DIGEST
    next_.verify.assert_called_once_with(f"{NEW_DIGEST}:1000000", "0xabc")
...
def test_verify_signed_digest_rejects_a_malformed_digest_before_verifying():
    ...
    with patch.object(svc, "match_validator_hotkey") as match:
