...
the miner did not. Each test drives the real pool through ``make_engine`` on a SQLite file: on ``main`` the first
raises ``ProgrammingError: Cannot operate on a closed database`` and the second hands back the same connection.
"""
...
def test_a_connection_that_died_while_idle_in_the_pool_is_replaced_not_handed_out(tmp_path):
    ...
    with engine.connect() as conn:  # main: sqlalchemy.exc.ProgrammingError — the dead connection was handed out
        assert conn.exec_driver_sql("SELECT 1").scalar() == 1
...
def test_a_connection_older_than_thirty_minutes_is_retired_on_checkout(tmp_path):
    ...
    with engine.connect() as conn:  # main: recycle is -1 ("never"), the same connection comes back
