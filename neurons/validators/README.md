...
- **Pass two (only when pass one verifies fewer than 3):** up to 300 more ports, spread evenly over the free declared ports pass one did not test, always including the highest. It uses the batch tier only, with one attempt, so it costs at most one extra container. It is skipped when pass one's batch container never ran, because a host that cannot start that container cannot run the test; the event then says so. The verified ports are pass one's plus pass two's.
