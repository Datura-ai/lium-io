...
- **Pass two (only when pass one verifies fewer than 3):** a port counts as verified after the DinD probe, which takes one answered port and keeps it only if it passes. Pass two probes up to 300 more ports, spread evenly over the free declared ports pass one did not test, always including the highest. It uses the batch tier only, with one attempt, so it costs at most one extra container. It is skipped when pass one's batch container didn't complete (it failed to start, timed out or stopped mid-test), because a host that cannot start that container cannot run the test; the event then says so. The verified ports are pass one's plus pass two's.
- **Pass two restores the port count only; it never overrides a failed DinD probe.** When pass one has an answer, the DinD probe runs first on it, exactly as before; if it fails, pass two does not run and the host keeps the old result (`skipped_container_failed`). When pass one has none, the probe runs after pass two on one of its answers; if it fails, none of pass two's answers count (`discarded_container_failed`, tallied with `counted: false`) and the host keeps the old result: 0 verified, DinD failed.
...
- **Per-range tally:** the port-connectivity event carries `port_ranges`, a list of `{pass, range, declared, probed, answered}` entries, and `second_pass`, which says whether pass two ran and why not (`not_needed`, `skipped_container_failed`, `skipped_batch_failed`, `no_ports_left`, `ran`, `batch_failed`, `discarded_container_failed`). Declared ports are grouped into buckets of `PORT_RANGE_BUCKET_WIDTH` (5000) ports by external port, so a partial forward shows which part of a wide range answered.
