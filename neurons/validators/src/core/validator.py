...
class Validator:
    def __init__(self):
        ...
        self.default_extra = {}
        # Designated-hotkey profile: the config-load refusal needs both hotkey lists; when the pool mirror
        # is empty only the operator can hold the dedicated-hotkey rule, and this line tells them.
        for warning in settings.designated_hotkey_startup_warnings():
            logger.warning(_m(f"[designated_hotkeys] {warning}", extra=get_extra_info(self.default_extra)))
