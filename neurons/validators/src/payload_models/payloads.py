...
class FailedContainerRequest(ContainerBaseResponse):
    ...
    # (docker_build), the timeout (build_timeout) or a one-line fixed reason for build_export and the
    # setup steps. Renter-safe: it is the output of the renter's own Dockerfile, never executor host
    # data. None for every other failure and from old validators.
