...
class ExecutorConnectivityService:
    ...
    async def verify_ports_bulk(
        ...
        container_name = f"{BATCH_VERIFIER_CONTAINER_PREFIX}_batch_check"

        logger.info(_m(f"batch: start docker {api_internal}:{api_external}", extra))

        try:
            # Clean up existing container with same name if exists
            await self._cleanup_container(ssh_client, container_name, extra)
    ...
    async def _cleanup_container(
        ...
        """
        Remove a Docker container by name (idempotent operation).

        Attempts to forcefully remove the specified container. If the container
        doesn't exist or was already removed, the error is silently logged at
        debug level. This ensures cleanup can be safely called multiple times.
    ...
    async def verify_port_dind(
        ...
        container_name = f"container_{miner_hotkey}_check_dind"
