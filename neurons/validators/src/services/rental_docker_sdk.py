...
class RentalDockerSdkClient:
    ...
    def _run_container_sync(self, spec: ContainerRunSpec) -> None:
        pod_network = self._ensure_rental_network_sync(spec.network) if spec.network else None
        ...
        if spec.inner_daemon_address_pools:
            self._seed_inner_daemon_config_sync(
                spec.name, spec.inner_daemon_address_pools, pod_network_subnets=_network_subnets(pod_network)
            )
        self._api_client.start(spec.name)

    def _seed_inner_daemon_config_sync(
        self,
        container_name: str,
        pools: tuple[AddressPool, ...],
        *,
        pod_network_subnets: tuple[str, ...] = (),
    ) -> None:
        ...
        Docker's own pools, and the log says why. So does a pod whose own network (the host daemon's
        `lium-rentals`) overlaps the pools: an inner network there would shadow the pod's address.
        """
        conflicts = pool_network_conflicts(pools, pod_network_subnets)
        if conflicts:
            outcome = f"skipped_pod_network_overlap: {'; '.join(conflicts)}"
        else:
            outcome = self._write_inner_daemon_pools_sync(container_name, pools)
        log = logger.warning if outcome.startswith(("failed", "skipped")) else logger.info
        log(
            _m(
    ...
    def _write_inner_daemon_pools_sync(self, container_name: str, pools: tuple[AddressPool, ...]) -> str:
        try:
            ...
            if existing is None:
                ...
                self._api_client.put_archive(
                    container_name,
                    posixpath.dirname(INNER_DAEMON_CONFIG_PATH),
                    _single_file_archive(INNER_DAEMON_CONFIG_PATH, merged, with_parent=False),
                )
            return "seeded"
    ...
    def _ensure_rental_network_sync(self, name: str) -> dict:
...
def _network_subnets(network: dict | None) -> tuple[str, ...]:
    """The IPAM subnets of an inspected Docker network."""
    ...
    if not isinstance(configs, list):
        return ()
    return tuple(config["Subnet"] for config in configs if isinstance(config, dict) and config.get("Subnet"))
