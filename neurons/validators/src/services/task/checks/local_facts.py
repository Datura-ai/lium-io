...
class LocalFactsCheck:
    ...
    async def _run(self, ctx: Context) -> CheckResult:
        ...
        if port is None:
            ...
            # network and there is nothing to tunnel to — this cycle's listings stay on SSH.
            return self._unavailable(
                ctx, "no_tunnel_port", "/version names no local_verify_port", capabilities=capabilities
            )
        try:
