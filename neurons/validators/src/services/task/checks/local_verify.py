...
from .gpu_usage import gpu_usage_check_will_read_the_live_card, lium_workload_containers
...
if TYPE_CHECKING:
    from services.matrix_validation_service import MatmulChallenge
    from services.verifyx_validation_service import VerifyXChallenge
...
@dataclass(frozen=True)
class _Call:
    """The GPU call `LocalVerifyCheck` judges this cycle: `pending` when it is already in flight
    since `LocalVerifyStartCheck` (phase 3), else the `client`, loopback `port` and signed `intent`
...
@dataclass(frozen=True)
class _Gate:
    """A reason not to make the call at all: `kind` is `skipped` (nothing to run) or `fallback`
    (the SSH path decides); `reason` is the metric label and `detail` the sentence for the log."""
...
def _gate_reason(ctx: Context) -> _Gate | None:
    """The reasons not to make the call at all, decided before any round trip. ONE list
    ...
    if settings.LOCAL_VERIFY_FIRST_PASS_ONLY and not ctx.config.unscored:
        ...
        return _Gate(
            "fallback", "not_first_pass", "not the first pass: the one-call path is first-pass only"
        )
    if ctx.config.validator_keypair is None:
        return _Gate("fallback", "no_keypair", "pipeline has no validator keypair to sign with")
    matmul_ssh_reason = _matmul_ssh_reason(ctx)
    if matmul_ssh_reason is not None and not ctx.config.verifyx_enabled:
        return _Gate("fallback", matmul_ssh_reason, "matmul on SSH and VerifyX off: nothing to run")
    return None
...
def _gpu_usage_check_will_read_the_live_card(ctx: Context) -> bool:
    ...
    return gpu_usage_check_will_read_the_live_card(
        ctx.state.gpu_details or [], ctx.state.gpu_processes or [], workload
    )
...
class LocalVerifyCheck:
        ...
        try:
            ...
            if pending is not None and not pending.consumed:
                ...
                if not pending.task.done():
                    pending.task.cancel()
                elif not pending.task.cancelled():
                    # nothing to cancel; read the exception so asyncio does not report it unretrieved
                    pending.task.exception()
                pending.close()
            raise
        if pending is not None and not pending.consumed:
    ...
    def _gate(self, ctx: Context) -> CheckResult | None:
        ...
        if gate.kind == "skipped":
            return self._skipped(ctx, gate.detail)
        return self._fallback(ctx, "call", gate.reason, gate.detail)
    ...
    def _prepare(self, ctx: Context) -> _Prepared | _PrepareFailed:
        ...
        try:
            if _matmul_ssh_reason(ctx) is None:
    ...
    async def _find_call(self, ctx: Context) -> _Call | CheckResult:
        """The call this check judges: the one in flight since `LocalVerifyStartCheck` (phase 3),
        else one to make now — `/version` from the facts call or a fresh read, then the challenges.
        A `CheckResult` is the reason there is no call this cycle (not advertised, no port, our
        own prepare failed), rendered as this check's own outcome."""
        pending = _pending_of(ctx)
        ...
        if advertised.local_verify_port is None:
            ...
            return self._fallback(ctx, "call", "no_tunnel_port", "/version names no local_verify_port")
        ...
        return _Call(
            ...
            client=client,
            port=advertised.local_verify_port,
    ...
    async def _run(self, ctx: Context, started: list[HealthCheckProbe]) -> CheckResult:
        ...
        call = await self._find_call(ctx)
        if isinstance(call, CheckResult):
            return call
        pending = call.pending
        matmul_challenge = call.matmul_challenge
        verifyx_challenge = call.verifyx_challenge
        ...
        try:
            ...
            try:
                if pending is None:
                    # Through the session the SSH checks run on: the executor answers loopback
                    # peers only.
                    answer = await call.client.verify(ctx.ssh, call.port, call.intent)
                else:
        ...
        if pending is not None:
            ...
            what["early_wait_ms"] = int((time.perf_counter() - wait_started_at) * 1000)
            self._metric(
                ctx,
                "consumed",
                "gpu_early",
...
class LocalVerifyStartCheck(LocalVerifyCheck):
    ...
    def _start(self, ctx: Context) -> CheckResult:
        ...
        if _gpu_usage_check_will_read_the_live_card(ctx):
        ...
        gate = _gate_reason(ctx)
        if gate is not None:
            return self._not_started(ctx, gate.reason, gate.detail)
        ...
        try:
            ...
            pending.task = asyncio.create_task(
                client.verify(ctx.ssh, facts.local_verify_port, prepared.intent),
                name="local_verify.gpu_early",
            )
