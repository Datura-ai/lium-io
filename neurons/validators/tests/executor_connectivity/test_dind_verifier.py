...
# ── B-176: a failed `docker run --gpus all` carries a reason code for the provider ──────────

NVML_MISMATCH_STDERR = (
    ...
    "error during container init: error running prestart hook #0: exit status 1, stdout: , stderr: "
...
async def test_dind_verifier_creation_failure_names_the_reason(mocker, stderr, expected_code):
    """Regression (ticket-0325): the probe failed with the NVML mismatch on every cycle for 2.5 h
    and only an ERROR log line on the validator said so. The result now carries the code and the
    daemon's text; an unrelated start failure gets the generic code (negative control)."""
    port = PortPair(9000, 9000)
    ssh_service = mocker.Mock()
    ssh_service.generate_keypair.return_value = ("priv", "pub")
    ssh_client = mocker.AsyncMock()
    ssh_client.run = mocker.AsyncMock(
        side_effect=[
            _run_result(mocker, exit_status=125, stderr=stderr),
            _run_result(mocker, exit_status=0, stdout=""),
        ]
    )
...
@pytest.mark.asyncio
async def test_dind_verifier_success_carries_no_reason(mocker):
    started = _build_started_dind(mocker)
    connection = mocker.AsyncMock()
    connection.run = mocker.AsyncMock(return_value=_run_result(mocker, exit_status=0))
    ...
    assert result.success is True
    assert result.reason_code is None and result.error is None
