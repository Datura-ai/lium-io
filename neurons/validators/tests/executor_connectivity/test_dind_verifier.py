...
def test_diagnose_docker_run_error_names_the_driver_mismatch_and_quotes_the_hook_line():
    code, words = diagnose_docker_run_error(NVIDIA_MISMATCH_STDERR)
    assert code == "NVIDIA_RUNTIME_MISMATCH"
    assert "kernel driver" in words and "different versions" in words
...
def test_diagnose_docker_run_error_generic_nvml_failure_and_none_for_the_rest():
    code, words = diagnose_docker_run_error(NVIDIA_GPU_RESET_STDERR)
    assert code == "NVIDIA_CONTAINER_HOOK_FAILED"
    assert words.endswith("docker said: nvidia-container-cli: detection error: nvml error: gpu requires reset")
...
def test_diagnose_docker_run_error_quotes_from_the_hook_and_caps_head_first():
    ...
    code, words = diagnose_docker_run_error(stderr)
    assert code == "NVIDIA_CONTAINER_HOOK_FAILED"
    assert "docker said: nvidia-container-cli: detection error: nvml error: xxx" in words
    assert "ppp" not in words
...
async def test_dind_verifier_docker_run_refused_by_the_nvidia_hook_carries_the_cause(mocker):
    ...
    assert result.error is not None and result.error.startswith("NVIDIA_RUNTIME_MISMATCH: ")
    assert "driver/library version mismatch" in result.error
    commands = [call.args[0] for call in ssh_client.run.await_args_list]
