...
def test_port_selector_from_mappings_excludes_ssh_and_rented():
    ...
    result = selector.select(info, size=5, unavailable_ports={9001}).ports
...
def test_port_selector_from_range_uses_range_list():
    ...
    selector = PortSelector()
    result = selector.select(info, size=2, unavailable_ports=set()).ports
...
def test_port_selector_default_range_when_missing():
    ...
    selector = PortSelector()
    result = selector.select(info, size=3, unavailable_ports=set()).ports
...
def test_port_selector_empty_when_all_rented():
    ...
    selector = PortSelector()
    result = selector.select(info, size=2, unavailable_ports={9000, 9001}).ports
