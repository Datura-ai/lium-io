...
# ------------------------------------------------------------------
# DAH-2211 review: BuildLogShipper lifecycle inside _custom_build_image.
# Pins that the validator ships build lines to backend's
# /pods/{pod_id}/build-log-chunk for §3.B.2 re-readability.
# ------------------------------------------------------------------


@pytest.mark.asyncio
async def test_build_log_shipper_lifecycle_on_success(svc, monkeypatch):
    """On the build-success path: a shipper is constructed, started, and
    `on_line` is threaded into execute_and_stream_logs. stop() is awaited.
    """
    ssh_client = AsyncMock()
    ssh_client.run = AsyncMock(return_value=_ssh_result())
    ...
    monkeypatch.setattr("services.build_log_shipper.BuildLogShipper", _FakeShipper)
    ...
    # The shipper.append must be wired as the on_line callback into the
    # build's execute_and_stream_logs call so each emitted line gets POSTed.
    assert callable(captured_kwargs.get("on_line"))


@pytest.mark.asyncio
async def test_build_log_shipper_stop_called_on_build_invocation_failure(svc, monkeypatch):
    """When execute_and_stream_logs raises mid-build, the finally must still
    stop() the shipper — no leaked task.
    """
    ssh_client = AsyncMock()
    ssh_client.run = AsyncMock(return_value=_ssh_result())

    async def _exec(**kwargs):
        raise RuntimeError("simulated SSH crash")
    ...
    assert ok is False
    assert step == "docker_build"
    # finally fires regardless of the raise.
    started.assert_awaited_once()
    stopped.assert_awaited_once()
