...
GPUs, the renter deleted his $1,023 rental himself 25 min later and the close was filed user_initiated.
...
async def test_second_rented_mismatch_cycle_clears_the_verified_job(context_factory):
    redis = FakeRedis()
    first = await GpuModelValidCheck().run(_ctx(context_factory, advertised=8, seen=1, pods=1, redis=redis))
    ...
    second = await GpuModelValidCheck().run(_ctx(context_factory, advertised=8, seen=1, pods=1, redis=redis))
    ...
    assert second.event.what_we_saw["first_seen_at"] == json.loads(redis.store[KEY])["first_seen_at"]
...
async def test_zero_gpus_on_a_rented_node_counts_too(context_factory):
    ...
    result = await GpuModelValidCheck().run(_ctx(context_factory, advertised=0, seen=0, pods=1, redis=redis))
    ...
    assert result.updates["clear_verified_job_evidence"]["plain_reason_code"] == "GPU_MODEL_UNSUPPORTED"
...
async def test_idle_node_mismatch_is_the_plain_failure_and_resets_the_streak(context_factory):
    ...
    result = await GpuModelValidCheck().run(_ctx(context_factory, advertised=8, seen=1, pods=0, redis=redis))
...
async def test_clean_scrape_resets_the_streak(context_factory):
    redis = FakeRedis()
    await GpuModelValidCheck().run(_ctx(context_factory, advertised=8, seen=1, pods=1, redis=redis))
    ok = await GpuModelValidCheck().run(_ctx(context_factory, advertised=8, seen=8, pods=1, redis=redis))
    assert ok.passed is True
    assert ok.event.reason_code == "GPU_MODEL_OK"
    assert KEY not in redis.store
    again = await GpuModelValidCheck().run(_ctx(context_factory, advertised=8, seen=1, pods=1, redis=redis))
...
async def test_threshold_is_the_setting(context_factory, monkeypatch):
    ...
    codes = []
    for _ in range(3):
        result = await GpuModelValidCheck().run(_ctx(context_factory, advertised=8, seen=1, pods=1, redis=redis))
...
async def test_no_redis_service_is_the_plain_failure(context_factory):
    result = await GpuModelValidCheck().run(_ctx(context_factory, advertised=8, seen=1, pods=1, redis=None))
    assert result.event.reason_code == "GPU_DETAILS_MISMATCH"
    assert "clear_verified_job_info" not in result.updates
