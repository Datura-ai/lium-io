...
async def test_provider_node_first_pass_still_sends_verifyx_in_the_one_call(
    ...
    """The provider-node twin of the test above: `designated_hotkey_first_pass` off (the default,
    and the only value a node not registered under a designated hotkey can have) keeps VerifyX in
    the intent."""
    validation = matmul_service(monkeypatch)
    async with FakeExecutor(keypair) as executor:
        ctx = context(
