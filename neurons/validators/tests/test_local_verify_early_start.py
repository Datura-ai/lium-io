...
from neurons.validators.src.services.task.checks.rental_verification import HealthCheckProbe
...
from services.local_verify_client import CAPABILITY
...
def context(keypair, executor_info, *, validation, verifyx, unscored=True, state=None, backend=None):
    ...
    return make_context(
        ...
        ssh=FakeSSH(),  # the tunnel the POST rides (#1340)
        ...
        state=with_tunnel_port(state or build_state(specs=SPECS, local_facts=ADVERTISED), executor_info),
    )


def with_tunnel_port(state, executor_info):
    """The fake serves `/verify` on its `/version` port; a fact table that advertises the capability
    names it as the loopback port the tunnel targets (#1340), the way `LocalFactsCheck` records it."""
    facts = state.local_facts
    if facts is None or CAPABILITY not in facts.capabilities or facts.local_verify_port is not None:
        return state
    return replace(state, local_facts=replace(facts, local_verify_port=executor_info.port))
...
@pytest.mark.asyncio
async def test_an_early_call_without_the_matmul_is_judged_with_the_reason_it_stayed_on_ssh(
    keypair, monkeypatch, early_on, verifyx_service
):
    """`_matmul_ssh_reason` (#1340) decides once for both checks: with the fast path off
    (`first_pass=False`, `unscored=True`, VerifyX on) the start check sends VerifyX alone, and the
    judge of the pending call labels the matmul fallback `scored_ssh` — the same line the late call
    writes — not a bare `not_asked`. Regression: the judge reading the reason from the challenge
    alone, so the early path lost the label main's metric counts by."""
    validation = matmul_service(monkeypatch)
    ssh_must_not_run(validation, verifyx_service)
    async with FakeExecutor(keypair) as executor:
        ctx = context(keypair, executor.executor_info, validation=validation, verifyx=verifyx_service)
        ctx = ctx.model_copy(update={"config": replace(ctx.config, first_pass=False, unscored=True)})
        start = await LocalVerifyStartCheck(client_factory=client_factory(keypair)).run(ctx)
        assert start.passed and start.event.reason_code == "LOCAL_VERIFY_EARLY_STARTED", start.event
        assert start.event.what_we_saw["steps"] == ["verifyx"]
    ...
    assert local.event.reason_code == "LOCAL_VERIFY_OK", local.event
    assert local.event.what_we_saw["consumed"] == ["verifyx"]
    assert local.event.what_we_saw["fallbacks"] == {"matmul": "scored_ssh"}
    assert "early_lead_ms" in local.event.what_we_saw
...
async def test_the_rental_probe_still_starts_where_the_judge_runs(keypair, monkeypatch, early_on, verifyx_service):
    ...
    probe = local.updates["state"].local_verify.health_check_probe
...
async def test_an_error_between_taking_the_call_and_awaiting_it_still_cancels_the_task(
    ...
    monkeypatch.setattr(settings, "LOCAL_VERIFY_RENTAL_PROBE_PARALLEL", True)
    monkeypatch.setattr(
        "neurons.validators.src.services.task.checks.local_verify.health_check_probe_request",
...
async def test_the_settle_step_handles_both_kinds_of_in_flight_work(monkeypatch):
    ...
    probe = HealthCheckProbe(task=asyncio.create_task(never()), request={})
    ctx = make_context(state=build_state(local_verify=LocalVerifyOutcome(pending=pending, health_check_probe=probe)))
    with MagicMock() as log:
        monkeypatch.setattr("neurons.validators.src.services.task.pipeline.logger", log)
        ...
        assert sorted((line["step"], line["reason"]) for line in outcome_lines(log)) == [
            ...
            ("health_check_probe", "unconsumed_cancelled"),
        ]
