...
OWN_SUPPLY_FAST_VALIDATION_ENABLED on. Skipped: the collateral read and VerifyX (both exist to hold
an outside provider to its claims). Resized: the capability matmul (kept, at the first-pass VRAM
budget). Kept whole: every check that catches a broken node. A provider node — whatever it reports
about itself — takes today's full pipeline; so does every scored cycle of the own-supply node.
"""
...
LIUM_HOTKEY = "lium-own-supply-hotkey-fixture-1"
OTHER_LIUM_HOTKEY = "lium-own-supply-hotkey-fixture-2"
PROVIDER_HOTKEY = "provider-hotkey-fixture"


@pytest.fixture
def profile_on(monkeypatch):
    monkeypatch.setattr(settings, "OWN_SUPPLY_FAST_VALIDATION_ENABLED", True)
    monkeypatch.setattr(settings, "OWN_SUPPLY_MINER_HOTKEYS", f"{LIUM_HOTKEY}, {OTHER_LIUM_HOTKEY}")
...
def test_flag_and_hotkeys_default_off():
    assert settings.OWN_SUPPLY_FAST_VALIDATION_ENABLED is False
    assert settings.OWN_SUPPLY_MINER_HOTKEYS == ""
    assert settings.own_supply_miner_hotkeys() == frozenset()
    assert settings.is_own_supply_trusted_first_pass(LIUM_HOTKEY, first_pass=True) is False
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, LIUM_HOTKEY, LIUM_HOTKEY, True, True),
        # flag off: nobody, even the Lium hotkey on its first pass
        (False, LIUM_HOTKEY, LIUM_HOTKEY, True, False),
        # flag on, empty list: nobody
        (True, "", LIUM_HOTKEY, True, False),
        # a provider's hotkey never matches
        (True, LIUM_HOTKEY, PROVIDER_HOTKEY, True, False),
        # a scored cycle (the wave never passes first_pass) never takes the profile
        (True, LIUM_HOTKEY, LIUM_HOTKEY, False, False),
        # a prefix / superstring of the Lium hotkey is not the Lium hotkey
        (True, LIUM_HOTKEY, LIUM_HOTKEY[:-1], True, False),
        (True, LIUM_HOTKEY, LIUM_HOTKEY + "0", True, False),
        (True, LIUM_HOTKEY, None, True, False),
        (True, LIUM_HOTKEY, "", True, False),
    ],
)
def test_profile_needs_flag_listed_hotkey_and_first_pass(
    ...
    monkeypatch.setattr(settings, "OWN_SUPPLY_MINER_HOTKEYS", hotkeys)
...
@pytest.mark.asyncio
async def test_build_context_marks_the_own_supply_first_pass_trusted(profile_on, monkeypatch):
    monkeypatch.setattr(pipeline_factory_module, "Context", lambda **kw: SimpleNamespace(**kw))
    ctx = await _build_context(LIUM_HOTKEY, first_pass=True)
    assert ctx.config.trusted_provider is True
...
@pytest.mark.asyncio
async def test_build_context_scored_cycle_of_the_own_supply_node_is_untrusted(
    profile_on, monkeypatch
):
    monkeypatch.setattr(pipeline_factory_module, "Context", lambda **kw: SimpleNamespace(**kw))
    ctx = await _build_context(LIUM_HOTKEY, first_pass=False)
    assert ctx.config.trusted_provider is False


@pytest.mark.asyncio
async def test_build_context_provider_with_every_trusted_looking_field_is_untrusted(
    profile_on, monkeypatch
):
    """The profile is keyed on the miner hotkey the validator authenticated. A provider node that
    reports every field a Lium node would — the same address family, root dir, username, even the
    Lium hotkey as its *executor* fields — still runs the full pipeline: nothing an executor
    reports is read."""
    monkeypatch.setattr(pipeline_factory_module, "Context", lambda **kw: SimpleNamespace(**kw))
    ctx = await _build_context(
        ...
        executor_overrides={
            "uuid": LIUM_HOTKEY,
            "miner_hotkey": LIUM_HOTKEY,
            "own_supply": True,
            "trusted": True,
            "provider_display_name": "Lium",
            "operated_by_lium": True,
        },
    )
    assert ctx.config.trusted_provider is False
...
@pytest.mark.asyncio
async def test_build_context_flag_off_keeps_the_lium_hotkey_untrusted(monkeypatch):
    monkeypatch.setattr(settings, "OWN_SUPPLY_FAST_VALIDATION_ENABLED", False)
    monkeypatch.setattr(settings, "OWN_SUPPLY_MINER_HOTKEYS", LIUM_HOTKEY)
    monkeypatch.setattr(pipeline_factory_module, "Context", lambda **kw: SimpleNamespace(**kw))
    ctx = await _build_context(LIUM_HOTKEY, first_pass=True)
    assert ctx.config.trusted_provider is False
...
def test_the_check_list_is_the_same_for_every_node():
    ...
    the checks that catch a broken node are on the list for an own-supply node as for any other."""
...
def _collateral_ctx(
    context_factory, trusted: bool, service: DummyCollateralService, enable_no_collateral=False
):
    specs = {"gpu": {"count": 8, "details": [{"name": "NVIDIA B300 SXM6 AC"}]}}
    return context_factory(
        services=build_services(collateral=service),
        config=build_context_config(
            trusted_provider=trusted, enable_no_collateral=enable_no_collateral
        ),
        state=build_state(specs=specs, gpu_count=8, gpu_details=specs["gpu"]["details"]),
        miner_hotkey=LIUM_HOTKEY if trusted else PROVIDER_HOTKEY,
    )


@pytest.mark.asyncio
async def test_own_supply_first_pass_skips_the_collateral_read(context_factory):
    service = DummyCollateralService(deposited=False, error="no bond", contract_version="1.0.2")
    result = await CollateralCheck().run(
        _collateral_ctx(context_factory, trusted=True, service=service)
    )

    assert result.passed is True
    assert result.event.reason_code == ColMsg.OWN_SUPPLY_SKIPPED.reason
...
async def test_provider_node_still_pays_the_collateral_read(context_factory):
    service = DummyCollateralService(deposited=False, error="no bond", contract_version="1.0.2")
    result = await CollateralCheck().run(
        _collateral_ctx(context_factory, trusted=False, service=service)
    )
...
def _verifyx_ctx(context_factory, trusted: bool, service: DummyVerifyXService):
    scrape_specs = {
        "gpu": {"count": 8},
        "ram": {"total": 2_000_000},
    ...
    return context_factory(
        ...
        config=build_context_config(verifyx_enabled=True, trusted_provider=trusted),
...
@pytest.mark.asyncio
async def test_own_supply_first_pass_skips_verifyx_and_publishes_the_scrape_readings(
    context_factory,
):
    service = DummyVerifyXService(success=True, updated_specs={"ram": {"total": 1}})
    ctx = _verifyx_ctx(context_factory, trusted=True, service=service)
    result = await VerifyXCheck().run(ctx)

    assert result.passed is True
    assert result.event.reason_code == VxMsg.OWN_SUPPLY_SKIPPED.reason
...
async def test_provider_node_still_runs_verifyx(context_factory):
    ...
    result = await VerifyXCheck().run(_verifyx_ctx(context_factory, trusted=False, service=service))
...
async def test_verifyx_disabled_wins_over_the_profile(context_factory):
    ...
    ctx = context_factory(
        services=build_services(verifyx=service),
        config=build_context_config(verifyx_enabled=False, trusted_provider=True),
...
def _capability_ctx(context_factory, trusted: bool, service):
    return context_factory(
        services=build_services(validation=service),
        config=build_context_config(trusted_provider=trusted),
        state=build_state(specs={"gpu": {"count": 8}}),
    )


@pytest.mark.asyncio
async def test_own_supply_first_pass_keeps_the_matmul_at_the_first_pass_budget(
    ...
    result = await CapabilityCheck().run(
        _capability_ctx(context_factory, trusted=True, service=service)
    )
...
@pytest.mark.asyncio
async def test_own_supply_first_pass_still_fails_a_card_that_cannot_compute(context_factory):
...
async def test_provider_node_matmul_fills_the_card(context_factory, monkeypatch):
    ...
    service = _SizingAwareValidationService(success=True)
    result = await CapabilityCheck().run(
        _capability_ctx(context_factory, trusted=False, service=service)
    )
    assert result.passed is True
...
async def test_gpu_count_gate_is_the_same_for_both(context_factory, trusted):
    ctx = context_factory(
        config=build_context_config(max_gpu_count=8, trusted_provider=trusted),
...
@pytest.mark.asyncio
@pytest.mark.parametrize("trusted", [True, False])
async def test_vram_gate_is_the_same_for_both(context_factory, trusted):
...
@pytest.mark.asyncio
@pytest.mark.parametrize("trusted", [True, False])
async def test_port_bind_gate_is_the_same_for_both(context_factory, trusted):
    ctx = context_factory(
        config=build_context_config(trusted_provider=trusted),
...
@pytest.mark.asyncio
@pytest.mark.parametrize("trusted", [True, False])
async def test_sysbox_gate_is_the_same_for_both(context_factory, trusted):
    ctx = context_factory(
        config=build_context_config(trusted_provider=trusted),
...
@pytest.mark.asyncio
@pytest.mark.parametrize("trusted", [True, False])
async def test_rental_verification_still_rents_the_probe_container_for_both(trusted):
...
def _score_ctx(
    trusted: bool, specs: dict, *, price_per_gpu=None, collateral_deposited=False, **flags
):
...
def test_own_supply_first_pass_scores_positive_without_verifyx_ema_or_collateral(monkeypatch):
    monkeypatch.setattr(
...
def test_own_supply_first_pass_keeps_every_broken_or_hostile_host_gate(flags, fragment):
...
def test_own_supply_first_pass_keeps_the_price_cap(monkeypatch):
    ...
    ctx = make_context(
        ...
        config=build_context_config(trusted_provider=True),
    )
    actual, job, warning = calculate_scores(ctx, rented=False)
    assert actual == 0.0
