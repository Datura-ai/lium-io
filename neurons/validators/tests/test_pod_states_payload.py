...
async def test_result_handler_never_sends_more_pod_states_than_the_backend_accepts() -> None:
    ...
    longer list fails its validation, dropping the whole spec. The checks share the list out under
    the bound (StaleContainerCleanupCheck reserves the rented pods' slots and hands the rest to its
    reaped queue in turns), so this cut is the last guard for the wire: it keeps the list's order
    and fires only on a rented list longer than the bound."""
    reaped = [
    ...
    assert result.pod_states == [*reaped, *observed][:256]
    ExecutorSpecRequest(**_spec_kwargs(), pod_states=result.pod_states)

    # under the bound nothing is touched, whatever the mix
    whole = await _job_result_for(make_context(state=build_state(pod_states=[*reaped, *observed[:246]])))
    assert whole.pod_states == [*reaped, *observed[:246]]
