...
async def test_filler_that_ended_its_own_run_is_reported_and_not_a_kill():
    """DAH-3475 cap exit: the Dolphin image exits 0 when no worker can serve, and `on-failure` leaves
    the container in `exited`. The backend must get the container_missing evidence so its reconciler
    closes the run, and the provider gets no penalty: the job ended itself, the host removed nothing.
    Under `unless-stopped` dockerd restarted it with fresh counters and this path was never reached."""
    backend_client = _killed_filler_backend()
...
async def test_filler_terminated_by_a_signal_is_a_kill():
    ...
    `restart: on-failure:5`, so a container that is still exited was stopped deliberately (a stop
    cancels the restart) or killed from outside past the retry budget — the policy would have
    brought back anything else. An image that traps SIGTERM and exits 0 (Dolphin's on_term) lands
    in the exited-on-its-own path instead. Prod: all 4 such runs in 7 days came from the one host
    with a container auto-killer."""
    backend_client = _killed_filler_backend()
...
async def test_container_running_again_by_the_time_we_inspect_is_not_a_death():
    """Filler containers run with `restart: on-failure:5`, so docker can bring a crashed one back
