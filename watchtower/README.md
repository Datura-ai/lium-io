...
5. If digests differ, checks that the host's NVIDIA runtime can start a GPU container: a throwaway container from the runner's own image with every GPU requested and the entrypoint `true` (nothing pulled, removed afterwards). The container carries the label `io.lium.watchtower.probe=nvidia-runtime`; a labelled leftover from an earlier cycle is removed before a new one is created, and a leftover that cannot be removed holds the update with no new container. If the probe cannot start, the cycle ends with `Update held` and a `reason_code` (`NVIDIA_RUNTIME_MISMATCH` when the daemon's message says `driver/library version mismatch`, else `RUNTIME_PROBE_FAILED`); the check repeats every cycle, so the update resumes once the host is fixed. A first boot with no runner skips this step.
...
A pull by tag goes through the host's Docker daemon, and the daemon asks a registry mirror first when `/etc/docker/daemon.json` has `registry-mirrors`. A mirror can keep serving an old copy of a tag. A pull by digest is content-addressed: the daemon checks the hash of the manifest it receives, so a mirror can return the right image or an error, never an old image. When that pull fails or returns an image without the digest, the same digest is pulled as `registry-1.docker.io/<image>@<digest>`. The daemon applies `registry-mirrors` to `docker.io` names only, so this reference reaches Docker Hub directly. The container is then created from whichever reference succeeded.
...
Every build is tagged twice: `daturaai/lium-watchtower:<env tag>` (`latest`, `staging`, `local`) and `daturaai/lium-watchtower:<version>` for prod or `<version>-<env tag>` for the others, where `<version>` is `project.version` in `pyproject.toml`. The executor compose files pull the version tag (`1.1.1` in `docker-compose.yml`, `1.1.1-staging` in `docker-compose.dev.yml`), and `neurons/executor/tests/test_stack_updater_by_digest.py` fails when the two drift.

The version tags are pushed by the GitHub Actions workflow `.github/workflows/watchtower_image.yml` ("CD: watchtower image"). Push a git tag `watchtower-v<version>` at the commit to publish, or run the workflow from the Actions tab with the tag (default: the current version) and `push_staging` on. The Actions tab lists the workflow only once the file is on `main`, so a version that must be on Docker Hub before its branch merges is published with the git tag. The job builds with this script, fails when the tag is not the version in `pyproject.toml`, refuses a tag that is already on Docker Hub (a version tag never moves; `overwrite` only finishes a run that failed halfway), and pushes `<version>` and `<version>-staging` with the repository's Docker Hub secrets. It does not push `latest` or `staging`. Bumping the version means: edit `pyproject.toml`, edit both compose files, push the release tag (or run the workflow) before the compose change merges.
