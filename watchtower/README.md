...
Every build is tagged twice: `daturaai/lium-watchtower:<env tag>` (`latest`, `staging`, `local`) and `daturaai/lium-watchtower:<version>` for prod or `<version>-<env tag>` for the others, where `<version>` is `project.version` in `pyproject.toml`. The executor compose files pull the version tag (`1.2.0` in `docker-compose.yml`, `1.2.0-staging` in `docker-compose.dev.yml`), and `neurons/executor/tests/test_stack_updater_by_digest.py` fails when the two drift.
